...
type Argument1<T extends Function> = T extends (a: infer U, ...b: any[]) => any ? U : never

// Client ID and API key from the Developer Console
var CLIENT_ID = '772721993565-c1es60fgnbb0p862cnmio0vfeguecuia.apps.googleusercontent.com';
...
function useGapi(api: string, initArgs: Argument1<typeof gapi.client.init>): typeof gapi | undefined {
  const script = document.getElementById('gapi-script')
  if (!script) throw new Error("The Google API script is not present or does not have an ID of gapi-script")
  ...
  useEffect(() => {
    ...
    if (window.gapi === undefined) script.addEventListener('load', setup, {once: true})
    else setup()
  }, [])
  return apiObj
}
